...
player = Player(
    position=pr.Vector2(100, height - int(dino_texture.height) - 20),
    texture=dino_texture,
    color=pr.WHITE,
    debug_color=pr.BLUE,
)  # spawn on floor
...
while not pr.window_should_close():
    ...
    if not player.dead:
        ...
        player.check_collisions_enemies(
            enemies=[x.get_rectangle() for x in cactus_list]
        )
    ...
    # draw floor
    pr.draw_line_v(
        pr.Vector2(0, height - floor_y_pos),
        pr.Vector2(width, height - floor_y_pos),
        pr.BLACK,
    )
    pr.draw_line_v(
        pr.Vector2(0, int(height / 2)), pr.Vector2(width, int(height / 2)), pr.RED
    )
