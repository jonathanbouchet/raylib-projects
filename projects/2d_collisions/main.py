...
while not pr.window_should_close():
    ...
    pr.draw_rectangle_pro(
        rect2, pr.Vector2(rect2.width / 2, rect2.height / 2), angle, pr.BLUE
    )
    ...
    rect2_pts = get_rotated_corners(
        pr.Vector2(rect2.x, rect2.y), pr.Vector2(rect2.width, rect2.height), angle
    )

    # pylygoncollision
    polygon1 = PolygonCollision.shape.Shape(
        vertices=[tuple([r.x, r.y]) for r in rect1_pts]
    )
    polygon2 = PolygonCollision.shape.Shape(
        vertices=[tuple([r.x, r.y]) for r in rect2_pts]
    )

    pr.draw_fps(0, 0)
    pr.draw_text(
        "AABB:" + str((AABB_collisions(r1=rect1, r2=rect2))), 0, 20, 20, pr.BLUE
    )
    pr.draw_text(
        "OBB:" + str(polygons_overlap_sat(rect1_pts, rect2_pts)), 0, 40, 20, pr.BLUE
    )
    ...
    # if (AABB_collisions(r1=rect1, r2=rect2)):
    #     print(f"r1x: {rect1.x},{rect2.x + rect2.width}")
    #     print(f"r1x + width: {rect1.x + rect1.width},{rect2.x }")
    #     print(f"r1y: {rect1.y},{rect2.y + rect2.height}")
    #     print(f"r1.y + height: {rect1.y + rect1.height},{rect2.y}")
    pr.draw_fps(0, 0)
    pr.end_drawing()
