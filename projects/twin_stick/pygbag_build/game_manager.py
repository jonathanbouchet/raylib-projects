...
from customer_timer import Timer
from states import GameStates, WaveStates
from explosion import Explosion
from utils import check_OOB_collisions
...
class GameManager:
    def __init__(self, resources_manager) -> None:
        ...
        self.asteroids: list[Asteroid] = [
            Asteroid(
                ...
                angle_rotation=random.uniform(
                    self.resources_manager.asteroid_data().get("angle_rotation")[0],
                    self.resources_manager.asteroid_data().get("angle_rotation")[1],
                ),
                window_borders=pr.Vector2(self.width, self.height),
                size=pr.Vector2(
    ...
    def load_sound_effects(self) -> None:
        ...
        self.laser_sound: pr.Sound = pr.load_sound(
            f"{self.resources_manager.sound_effect_data().get('laser')}"
        )
        self.explosion_sound: pr.Sound = pr.load_sound(
            f"{self.resources_manager.sound_effect_data().get('explosion')}"
        )
    ...
    def init(self) -> None:
        ...
        # )
        # self.shader = pr.load_shader(pr.ffi.NULL, f"{self.shader_bloom}")
    ...
    def check_collisions(self) -> None:
        ...
        for laser in self.player.lasers:
            for asteroid in self.asteroids:
                ...
                if check_OOB_collisions(laser_rect, asteroid_rect):
    ...
    def draw_init(self) -> None:
        ...
        pr.draw_text(
            ...
            int(120), int(200), 10, pr.GREEN)
        
        pr.draw_text(
            """
            RULES: AT EACH WAVE, CHOOSE BETWEEN MORE ASTEROIDS OR LESS TIME
            """, 
            int(50), int(220), 10, pr.GREEN)
        pr.draw_text(
            """
            REWARD FOR EACH WAVE = ASTEROIDS DESTROYED
            """, 
            int(110), int(240), 10, pr.GREEN)
        pr.draw_text(
            """
            CONTROLS: < ^ > TO MOVE THE SHIP, SPACE TO SHOOT
            """, 
            int(100), int(260), 10, pr.GREEN)
        pr.draw_text(
            """PRESS ENTER TO START""", 
            int(230), int(320), 10, pr.GREEN)
        
        # is_ok = pr.Rectangle(250, 350, 100, 30)
        # pr.draw_rectangle_rec(is_ok, pr.GREEN)
        # pr.draw_text("START", 265, 355, 20,pr.BLACK)

        # if pr.check_collision_point_rec(pr.get_mouse_position(), is_ok):
        #     pr.draw_rectangle_rec(is_ok, pr.DARKGREEN)
        #     pr.draw_text("START", 265, 355, 20,pr.BLACK)

            # if pr.is_mouse_button_pressed(1):

        enter_triggered = pr.is_key_down(rl.KEY_ENTER)
        if enter_triggered:
    ...
    def draw_sucess_wave(self) -> None:
        ...
        if self.state == GameStates.PAUSE:
            ...
            pr.draw_text(
                ...
                """, 
                int(150), int(220), 10, pr.GREEN)
            pr.draw_text(
                """
                <-    ASTEROID + 1\n
                ->    TIME - 1
                """, 
                int(150), int(240), 10, pr.GREEN)
            
            if pr.is_key_pressed(rl.KEY_LEFT) or pr.is_key_pressed(rl.KEY_RIGHT) :
                ...
                elif pr.is_key_pressed(rl.KEY_RIGHT):
                    self.scorer.next_wave(
                        user_choice=2,
                        current_asteroids=current_asteroids,
                        current_remaining_time=current_remaining_time,
    ...
    def fail_wave_screen(self) -> None:
        ...
        pr.draw_text(
                ...
                WAVE FAILED
                """, 
                int(190), int(220), 10, pr.GREEN)
        pr.draw_text(
            """
            PRESS START TO RESTART
            """, 
            int(170), int(240), 10, pr.GREEN)
